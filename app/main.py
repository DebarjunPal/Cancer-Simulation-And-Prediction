...
def load_data():
    data = pd.read_csv("D:\Cancer-Simulation-And-Prediction\data\data.csv")
    return data

data = load_data()

def get_clean_data():
    df = pd.read_csv(r"D:\Cancer-Simulation-And-Prediction\data\data.csv")
...
def display_predictions(input_data):
    model = pickle.load(open("D:\Cancer-Simulation-And-Prediction\model\model.pkl", "rb"))
    scaler = pickle.load(open("D:\Cancer-Simulation-And-Prediction\model\scaler.pkl", "rb"))
...
def main():
    ...
    with st.container():
        ...
        st.write("Unleash the future of health with EvoHealth which predicts using a machine learning model whether a breast mass is benign or malignant based on the measurements it receives from your cytosis lab. You can also update the measurements by hand using the sliders in the sidebar."
        "It offers a powerful platform to accelerate discovery by simulating cancer growth with unprecedented accuracy.")
